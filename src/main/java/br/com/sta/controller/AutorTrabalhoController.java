...
import br.com.sta.service.AutorTrabalhoService;
...
public class AutorTrabalhoController {
	...
	@Operation(summary = "Busca autorTrabalhos", description = "Buscar todos os autorTrabalhos, buscar autorTrabalhos por descricao", tags = {"autorTrabalho"})
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Sucesso",
					content = @Content(schema = @Schema(implementation = AutorTrabalho.class))),
	...
	public ResponseEntity<Page<AutorTrabalhoDTO>> findAutorTrabalhoByIdAutor(@PathVariable long id) {
		Page<AutorTrabalhoDTO> autorTrabalhos = autorTrabalhoService.findAllByIdAutor(id);
		return ResponseEntity.ok(autorTrabalhos);
	}

	@Operation(summary = "Busca ID", description = "Buscar autorTrabalho por ID do trabalho", tags = {"autorTrabalho"})
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Sucesso",
					content = @Content(schema = @Schema(implementation = AutorTrabalho.class))),
			@ApiResponse(responseCode = "404", description = "AutorTrabalho não encontrado para este autor")
	...
	public ResponseEntity<Page<AutorTrabalhoDTO>> findAutorTrabalhoByIdTrabalho(@PathVariable long id) {
		Page<AutorTrabalhoDTO> autorTrabalhos = autorTrabalhoService.findAllByIdTrabalho(id);
		return ResponseEntity.ok(autorTrabalhos);
	}
	
	@Operation(summary = "Adicionar autorTrabalho", description = "Adicionar novo autorTrabalho informado no banco de dados", tags = {"autorTrabalho"})
	@PostMapping(value = "/autorTrabalho")
	@CrossOrigin("http://localhost:3000")
	public ResponseEntity<AutorTrabalhoDTO> addAutorTrabalho(@RequestBody AutorTrabalho autorTrabalho) throws URISyntaxException {
		try {
			AutorTrabalho novoAutorTrabalho = autorTrabalhoService.save(autorTrabalho);
			
			return ResponseEntity.created(new URI("/api/autorTrabalho" + novoAutorTrabalho.getId())).body(new AutorTrabalhoDTO().converter(autorTrabalho));
	...
	@Operation(summary = "Alterar AutorTrabalho", description = "Alterar valores do autorTrabalho com id selecionado", tags = {"autorTrabalho"})
	...
	@Operation(summary = "Deletar autorTrabalho", description = "Deletar autorTrabalho com o ID informado", tags = {"autorTrabalho"})
